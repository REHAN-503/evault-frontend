        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-line bg-white/50">
              <h3 className="font-display text-lg font-medium">Upload Document</h3>
              ...
              {phase === 'form' && (
                <form onSubmit={handleSubmit}>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-slate mb-1.5 font-medium">Document Title</label>
                      <input required value={title} onChange={(e) => setTitle(e.target.value)}
                        placeholder="e.g. Affidavit of Evidence"
                        className="w-full rounded-md border border-line bg-white px-3.5 py-2.5 text-sm focus:border-seal focus:ring-1 focus:ring-seal outline-none transition-shadow" />
                    </div>

                    <div>
                      <label className="block text-xs uppercase tracking-wider text-slate mb-1.5 font-medium">Case Number</label>
                      <input required value={caseNo} onChange={(e) => setCaseNo(e.target.value)}
                        placeholder="e.g. CIV/2026/0417"
                        className="w-full rounded-md border border-line bg-white px-3.5 py-2.5 text-sm focus:border-seal focus:ring-1 focus:ring-seal outline-none transition-shadow font-mono" />
                    </div>

                    <div>
                      <label className="block text-xs uppercase tracking-wider text-slate mb-1.5 font-medium">File</label>
                  ...
                  <div className="flex items-center justify-end gap-3 mt-8">
                    ...
                    <button type="submit" disabled={!file || !title || !caseNo} className="rounded-md bg-ink text-paper px-6 py-2 text-sm font-medium hover:bg-ink-2 transition-colors disabled:opacity-50">
                      Upload
                    </button>
                  </div>
                </form>
                ...
                <div className="py-2">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="font-display text-xl">
                      {phase === 'running' ? 'Uploading...' : 'Upload Complete'}
                    </h3>
                  </div>
                  ...
                  {phase === 'done' && result && (
                    <div className="rounded-lg border border-line bg-white/70 p-4 flex items-center gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-slate font-mono mt-0.5 truncate">{result.docId}</p>
                      </div>
                    </div>
                  )}
